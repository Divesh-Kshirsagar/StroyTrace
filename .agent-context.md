...
Tasks 1.1–1.6, 2.1–2.4 complete (feature/secure-evidence-upload):
...
Next: Task 2.5 — update `EvidenceSchema` in `schemas.py` to expose `upload_status`. Branch: `feature/secure-evidence-upload`. Note: `pyvips` requires native `libvips` OS library — not present in dev env yet.
