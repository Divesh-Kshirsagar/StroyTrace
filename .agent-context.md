...
Tasks 1.1, 1.2, and 1.3 complete (feature/secure-evidence-upload):
...
- 1.2: Created `apps/backend/config/celery.py` with `Celery("config")` app instance, `os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings.base")`, and `app.config_from_object("django.conf:settings", namespace="CELERY")`. Note: `autodiscover_tasks` deferred to Task 5.3. Settings path `config.settings.base` will be created in Task 1.4 (splitting flat `config/settings.py` into `config/settings/base.py`).
...
Branch: `feature/secure-evidence-upload`. Note: `pyvips` requires native `libvips` OS library — not present in dev env yet. 40/40 backend tests pass.
