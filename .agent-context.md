...
Task 1.1 complete (feature/secure-evidence-upload): Added `boto3>=1.38.0,<1.39`, `celery>=5.4.0,<5.5`, `redis>=5.2.0,<5.3`, `python-magic==0.4.27`, `pyvips>=2.2.0,<2.3` to `apps/backend/pyproject.toml` as pinned runtime dependencies. `uv sync` resolved all packages. 40/40 backend tests pass. Branch: `feature/secure-evidence-upload`. Note: `pyvips` requires native `libvips` OS library (`libvips.so.42`) — not present in this dev env yet; Python package installed correctly.
