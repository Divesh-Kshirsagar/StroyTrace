...
**feature/secure-evidence-upload — ALL TASKS COMPLETE.** Commit: `9ad81ed`.

Completed this session:
- 6.3: `apps/backend/apps/events/tests/test_tasks.py` — 4 Celery task tests using moto/pyvips mocks. 51/51 backend tests pass.
- 7.1: Added `@uppy/core@4.4.2`, `@uppy/aws-s3@4.1.0`, `@uppy/react@4.1.2`, `@uppy/dashboard@4.2.0` to `apps/frontend/package.json`.
- 7.2: Created `apps/frontend/features/events/components/EvidenceUploader.tsx` — two-tab modal (URL + Uppy file upload). Calls `/evidence/upload-url` via `getUploadParameters`, then `/confirm-upload` on Uppy `upload-success`. Client-side restrictions: 5 MB max, allowed types: jpeg/png/webp/pdf.
- 7.3: Updated `EvidenceBoardPanel.tsx` to use `EvidenceUploader` instead of `AddEvidenceModal`.
- 8.1: Added `upload_status?: string` to generated `EvidenceSchema` in `apps/frontend/generated/types.gen.ts`.
- 8.2: Rewrote `apps/frontend/features/events/components/EvidenceBoard.tsx`: spinner for `processing`, error+remove for `failed`, full preview for `processed`/`url_based`, publicView filter (R6.4), 3-second polling hook.
- 9.1: `apps/frontend/features/events/components/__tests__/EvidenceUploader.test.tsx` — 8 tests; maxFileSize, allowedFileTypes, upload-url fetch call verified.
- 9.2: `apps/frontend/features/events/components/__tests__/EvidenceBoard.test.tsx` — 7 tests; processing/failed/processed states, public view filtering.
- 10: Ledger + context updated. JWT fix applied to `test_confirm_upload.py` and `test_upload_url.py`.

Branch cleanup is due: clear `docs/todo.md` and `docs/requirements.md`, record in `docs/agent_ledger.md`.
