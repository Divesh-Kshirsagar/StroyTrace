...
def test_search_events(api_client):
    ...
    EventFactory(title="Test Event Alpha", status="published", lead_investigator=profile.user)
    EventFactory(title="Test Event Beta", status="draft", lead_investigator=profile.user)
