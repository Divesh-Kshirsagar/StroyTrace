...
function makeEvidence(
  overrides: Partial<EvidenceSchema> & { upload_status?: string },
): EvidenceSchema {
  return {
...
describe("EvidenceBoard", () => {
  ...
  describe("upload_status = processing", () => {
    it("renders a spinner and no <img> or <a> tag", () => {
      ...
      // Card is in the DOM
      expect(
        screen.getByTestId("evidence-card-processing"),
      ).toBeInTheDocument();
  ...
  describe("upload_status = failed", () => {
    ...
    it("calls onRemove with the correct evidence ID when remove button is clicked", () => {
      const onRemove = vi.fn();
      const evidence = [
        makeEvidence({ id: "ev-failed-1", upload_status: "failed" }),
      ];
      render(
        <EvidenceBoard
          evidence={evidence}
      ...
      fireEvent.click(
        screen.getByRole("button", { name: /remove failed evidence/i }),
      );
  ...
  describe("upload_status = url_based", () => {
    it("renders the media card with image and link", () => {
      ...
      render(<EvidenceBoard evidence={evidence} publicView={false} />);
    ...
    it("hides processing and failed evidence in public view", () => {
      const evidence = [
        ...
        makeEvidence({
          id: "done",
          upload_status: "processed",
          caption: "Visible item",
        }),
      ];
      render(<EvidenceBoard evidence={evidence} publicView={true} />);
