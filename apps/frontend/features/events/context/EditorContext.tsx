...
export const EditorProvider = ({
  children,
  initialEvent = null,
}: { children: ReactNode; initialEvent?: EventFullSchema | null }) => {
  const [event, setEvent] = useState<EventFullSchema["event"] | null>(
    initialEvent?.event || null,
  );
  const [narrative, setNarrative] = useState({
    content: initialEvent?.narrative?.content || "",
    isDirty: false,
    isSaving: false,
  });
  const [evidenceQueue, setEvidenceQueue] = useState<EvidenceSchema[]>(
    initialEvent?.evidence || [],
  );
  ...
  const removeEvidence = async (id: string) => {
    ...
    if (isEditing && event) {
      try {
        ...
        await appsEventsRoutersDeleteEvidence({
          slug: event.slug,
          evidence_id: id,
        } as any);
      } catch (err) {
  ...
  const updateEvidenceOrder = async (newOrder: EvidenceSchema[]) => {
    ...
    if (isEditing && event) {
      try {
        const { appsEventsRoutersReorderEvidence } = await import(
          "@/generated"
        );
