...
import {
  appsEventsRoutersDeleteEvent,
  appsEventsRoutersUpdateEventStatus,
} from "@/generated";
...
export default function EventRow({ event, onUpdate }: EventRowProps) {
  ...
  const getStatusBadge = () => {
    switch (event.status) {
      case "published":
        return (
          <Badge className="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 border-none">
            Published
          </Badge>
        );
      case "draft":
        return (
          <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300 border-none">
            Draft
          </Badge>
        );
      case "archived":
        return (
          <Badge className="bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300 border-none">
            Archived
          </Badge>
        );
      default:
              ...
              <DropdownMenuItem
                ...
                onClick={() => {
                  if (event.status === "published")
                    window.open(
                      `/@${event.lead_investigator.handle}/${event.slug}`,
                      "_blank",
                    );
                }}
                className="flex items-center cursor-pointer"
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              event "{event.title}", including its narrative and all evidence.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
