...
export default function SubscribeForm({ channelHandle }: { channelHandle: string }) {
  ...
  useEffect(() => {
    try {
      const subs = JSON.parse(localStorage.getItem('clarity_subscriptions') || '{}');
  ...
  if (status === 'success') {
    return (
      <div className="bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-400 p-4 rounded-xl border border-green-200 dark:border-green-800/30 flex items-center gap-3">
        ...
        <span className="font-medium">You are subscribed to @{channelHandle}. We'll email you when new investigations are published.</span>
      </div>
    );
  }

  return (
    <form onSubmit={handleSubscribe} className="bg-zinc-50 dark:bg-zinc-900/50 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800/50">
      <h3 className="text-lg font-bold mb-2">Subscribe to @{channelHandle}</h3>
      <p className="text-sm text-zinc-500 mb-4">Get notified directly in your inbox when new investigations drop.</p>
