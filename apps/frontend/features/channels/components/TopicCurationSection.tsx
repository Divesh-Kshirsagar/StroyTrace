...
export default function TopicCurationSection({ channelHandle, initialTopics }: TopicCurationSectionProps) {
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Curated Topics</h3>
        {isOwner && (
          <Button variant="ghost" size="sm" onClick={handleOpenModal} className="h-6 text-xs px-2 py-0">
            Edit Topics
          </Button>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {topics.length > 0 ? (
          ...
          <p className="text-zinc-500 text-sm italic">This creator hasn't curated any topics yet.</p>
        )}
      </div>
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Select up to 10 topics that you cover. These will be displayed on your public profile.
            </DialogDescription>
          </DialogHeader>
          
          <div className="flex flex-col gap-3 my-4">
            {allTopics.map(topic => (
              <label key={topic.slug} className="flex items-start gap-3 p-3 border rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-900 cursor-pointer transition-colors border-zinc-200 dark:border-zinc-800">
                <Checkbox 
