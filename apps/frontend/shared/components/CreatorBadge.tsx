...
export default function CreatorBadge({ creator, className = '' }: CreatorBadgeProps) {
  return (
    <Link 
      href={`/@${creator.handle}`} 
    ...
    >
      <div className="w-6 h-6 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center overflow-hidden flex-shrink-0">
        {creator.avatar_url ? (
          ...
          <span className="text-[10px] font-medium text-zinc-500">{creator.display_name.charAt(0).toUpperCase()}</span>
        )}
      </div>
