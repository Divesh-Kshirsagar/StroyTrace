...
export async function generateMetadata({
  ...
  try {
    ...
    const { data } = await appsEventsRoutersGetEvent({
      path: { slug: eventSlug },
    } as any);
    ...
    const description =
      ...
      `A detailed investigation into ${event.title}...`;

    // Find primary thumbnail
    const canonicalUrl = `https://clarity.com/${handle}/${eventSlug}`;

    const firstEvidenceWithThumbnail = data.evidence?.find(
      (e) => e.thumbnail_url,
    );
    const ogImage =
      firstEvidenceWithThumbnail?.thumbnail_url || "/og-default.png";
    return {
      title,
      description,
      alternates: {
...
export default async function EventPage({ params }: PageProps) {
  ...
  let data: any;
  try {
    const response = await appsEventsRoutersGetEvent({
      path: { slug: eventSlug },
    } as any);
    data = response.data;
  } catch (error) {
