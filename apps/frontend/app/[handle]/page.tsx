...
import {
  appsFeedsRoutersChannelFeed,
  appsUsersRoutersGetChannel,
} from "@/generated";
...
export default async function ChannelPage({ params }: PageProps) {
  ...
  try {
    ...
    const { data: profileData } = await appsUsersRoutersGetChannel({
      path: { handle: cleanHandle },
    } as any);
    profile = profileData;

    const { data: feedData } = await appsFeedsRoutersChannelFeed({
      path: { handle: cleanHandle },
    } as any);
  ...
  async function fetchNextPage(cursor: string) {
    "use server";
    const response = await appsFeedsRoutersChannelFeed({
      path: { handle: cleanHandle },
      query: { cursor },
    } as any);
  ...
  return (
    <main>
        ...
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center overflow-hidden flex-shrink-0">
            {profile.avatar_url ? (
              ...
              <span className="text-3xl font-medium text-zinc-500">
      ...
      <FeedView
        ...
        header={
          <h2 className="text-xl font-semibold mb-2">Published Events</h2>
        }
